...
app.get("/api/movies/:page_id", async (req, res) => {
  const response = await fetch(
    `https://kinobd.xyz/api/films?page=${req.params.page_id}&with=persons%2Cgenres%2Ccountries%2Cpopularity`
  );
...
app.get("/api/movies/search/:query/:page_id", async (req, res) => {
  const response = await fetch(
    `https://kinobd.xyz/api/films/search/title?q=${req.params.query}&page=${req.params.page_id}&with=persons%2Cgenres%2Ccountries%2Cpopularity`
  );
